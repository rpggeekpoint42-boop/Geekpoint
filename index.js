...
async function startBot(){
  ...
  sock.ev.on("connection.update", async ({connection,lastDisconnect})=>{
    ...
    if(!sock.authState.creds.registered && !jaGerouCodigo){
      jaGerouCodigo = true
      const numero = "559180305171" // SEU NÚMERO SEM +, -, () ou espaços
      try {
  ...
  sock.ev.on("messages.upsert", async ({messages})=>{
    ...
    if(text.startsWith(`${prefix}ping`)){
      ...
      const sent = await sock.sendMessage(chatId,{ text:"🏓 Calculando ping..." })
      const latency = Date.now() - start
      await sock.sendMessage(chatId,{ edit: sent.key, text:`🏓 Pong!\n⚡ ${latency} ms` })
      return
    }
...
`
    ...
    if(text.startsWith(`${prefix}rename`)){
      ...
      if(!nickData[senderJid]){
        nickData[senderJid] = { atual:newNick, historico:[{nick:newNick,dataHora:now}] }
      } else {
        nickData[senderJid].historico.push({ nick:nickData[senderJid].atual, dataHora:now })
    ...
    if(text.startsWith(`${prefix}historico`)){
      ...
      const ultimaDataHora = user.historico[user.historico.length-1]?.dataHora || "N/A"
      const msgText = `
📝 Último Nick salvo
